...
description: Find the underspecified parts of a feature spec by asking up to 5 targeted questions, then encode the answers back into spec.md. Use after speckit-specify and before speckit-plan, whenever requirements may be ambiguous.
metadata:
...
1. Run `../speckit/scripts/feature-paths.sh --require spec --json` from the repo root **once** and parse the JSON payload:
   - `FEATURE_DIR`
   - `SPEC`
...
8. Write the updated spec back to `SPEC`.
...
Context for prioritization: whatever the user said when invoking this skill.
...
**Next:** `speckit-plan` once the spec has no outstanding `[NEEDS CLARIFICATION]` markers.

## Adapted from upstream

Reads and writes `specs/<slug>/spec.md` resolved from the branch.
