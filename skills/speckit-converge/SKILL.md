...
description: Assess what a feature's code actually does against its spec, plan, and tasks, then append the remaining unbuilt work to tasks.md. Use after an implementation pass to find drift between what was specified and what exists.
metadata:
...
**Constitution Authority**: The project constitution (`.agents/constitution.md`) is
...
- CONSTITUTION = `.agents/constitution.md` (if present)
...
**Next:** `speckit-implement` on the appended tasks, or close the feature out if nothing was appended.

## Adapted from upstream

Appends to `specs/<slug>/tasks.md`; does not rewrite existing tasks.
