...
description: Rewrite ambiguous English into ASD-STE100 Simplified Technical English — one meaning per word, active voice, simple tense, one instruction per sentence, short sentences. Use when a tool description, error message, system prompt, inter-agent instruction, or any text a machine or non-native reader must parse without a human is dense or ambiguous.
metadata:
  upstream: https://github.com/danyuchn/asd-ste100-skill @ 8564f89
  license: MIT (see LICENSE.upstream)
  related-skills: prose-reviewer
---
...
This skill is not for creative or marketing copy — STE is deliberately flat and literal. Do not apply it to text where voice, nuance, or persuasion is the point. For user-facing prose where voice matters, use `prose-reviewer` instead.
...
Imported from [danyuchn/asd-ste100-skill](https://github.com/danyuchn/asd-ste100-skill) (commit `8564f89`), MIT licensed — see `LICENSE.upstream`. Adapted here for this catalog's conventions: skill name matched to the directory, `metadata:` frontmatter, and the upstream repository README dropped (its install instructions do not apply to an Outfitter catalog).
