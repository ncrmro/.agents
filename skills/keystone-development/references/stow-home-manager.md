...
before stow links the replacement (Hyprland writes its autogenerated
config; stow then aborts on "existing target is not owned by stow").
Recovery: atomically `ln -sfn` the exact relative symlink stow would
create (stow adopts it as its own), then restart
